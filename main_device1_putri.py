# file name: main_device1_putri_signature.py
# file content begin
# main_device1_putri_signature.py - DEVICE 1 (Putri) dengan Signature hanya pada RSA
...
class DevicePutriSignature:
    ...
    def send_des_message(self):
        """Kirim pesan DES (TANPA SIGNATURE)"""
        if not self.des_mode:
            print("✗ Belum masuk mode DES. Selesaikan handshake RSA dengan signature terlebih dahulu.")
            return
        
        print("\n" + "="*50)
        print("🔓 KIRIM PESAN DES (TANPA SIGNATURE)")
        print("="*50)
    ...
    def start_des_communication(self):
        ...
        print("="*60)
        print("Fitur:")
        print("  ✓ Enkripsi/Dekripsi menggunakan DES")
        print("  ✗ TANPA digital signature")
        print("  ✗ TANPA message integrity verification")
        print("  ✗ TANPA non-repudiation")
        print("="*60)
        print("Ketik pesan DES atau pilih menu:")
    ...
    def show_protocol_status(self):
        """Tampilkan status protokol"""
        print("\n" + "="*50)
        print("STATUS PROTOKOL")
        print("="*50)
        print(f"Device: {self.name}")
        print(f"Role: {'Initiator' if self.is_initiator else 'Responder'}")
        print(f"Mode: {'DES (Tanpa Signature)' if self.des_mode else 'RSA (Dengan Signature)'}")
        print(f"Handshake Stage: {self.handshake_stage}")
        
        if self.des_mode:
            print("\n🔓 DES MODE (TANPA SIGNATURE):")
            print("  ✓ Enkripsi simetris cepat")
            print("  ✗ Tidak ada verifikasi integrity")
            print("  ✗ Tidak ada non-repudiation")
        else:
            print("\n🔐 RSA MODE (DENGAN SIGNATURE):")
            print("  ✓ Digital signature aktif")
            print("  ✓ Message integrity verification")
            print("  ✓ Non-repudiation")
        
        if 'secret_key' in self.session_data:
            key_len = len(self.session_data['secret_key'])
            print(f"\nSecret Key: {'*' * key_len} ({key_len} karakter)")
        else:
            print(f"\nSecret Key: BELUM DITERIMA")
    ...
    def show_des_menu(self):
        """Tampilkan menu DES"""
        print("\n" + "="*30)
        print("MENU DES (TANPA SIGNATURE) - PUTRI")
        print("="*30)
        print("[5] Kirim Pesan DES")
        print("[6] Tampilkan Status Protokol")
        print("[9] Keluar")
        print("="*30)
